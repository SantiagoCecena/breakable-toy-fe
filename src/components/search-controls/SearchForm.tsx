...
function SearchForm({ setFilters }: { setFilters: React.Dispatch<React.SetStateAction<Filters>> }) {
    ...
    return (
        <form className="flex flex-col gap-3 my-2 p-3 pt-4 rounded-md shadow-lg w-full max-w-4xl" onSubmit={handleSubmit}>
            <Input label="Name" placeholder="Take the dog out..." id="name" name="name" />

            <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
                <div className="flex flex-col">
                    <Select label="Priority" defaultValue="all" name="priority" classname="sm:min-w-[320px]">
                        <option value="all">All</option>
                        <option value="high">High</option>
                        <option value="medium">Medium</option>
                        <option value="low">Low</option>
                    </Select>
                    <Select label="State" defaultValue="all" name="state" classname="sm:min-w-[320px]">
                        <option value="all">All</option>
